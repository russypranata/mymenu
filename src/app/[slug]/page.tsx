  ...
  return (
    <CartProvider storeId={store.id}>
      <div className={`min-h-screen ${pageBg} ${fontClass}`}>
        ...
        {/* ── Menu Section ── */}
        <section id="menu" className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-14 pb-32">
          <div className="mb-10">
        ...
        {/* ── Footer ── */}
        <footer className={`border-t ${isDark ? 'border-slate-800 bg-slate-900' : 'border-slate-100 bg-white'} mt-12`}>
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                ...
                <div className="flex items-center gap-3">
                  {store.store_settings?.logo_url ? (
                    <div className="relative w-12 h-12 rounded-xl overflow-hidden flex-shrink-0">
                      <Image src={store.store_settings.logo_url} alt={store.name} fill sizes="48px" className="object-cover" />
                    </div>
                  ) : (
                    <div className="w-12 h-12 rounded-xl flex items-center justify-center text-lg font-bold text-white flex-shrink-0" style={{ backgroundColor: primaryColor }}>
              ...
              {/* Contact Info */}
              <div className="space-y-4">
                <h4 className={`text-sm font-bold uppercase tracking-wider ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>Kontak</h4>
                  ...
                  <div className="flex items-center gap-3 pt-1">
                    {instagram && (
                      <a href={`https://instagram.com/${instagram}`} target="_blank" rel="noopener noreferrer"
                        className={`w-9 h-9 rounded-xl flex items-center justify-center transition-colors ${isDark ? 'bg-slate-700 hover:bg-slate-600' : 'bg-slate-100 hover:bg-slate-200'}`} aria-label="Instagram">
                    ...
                    {facebook && (
                      <a href={`https://facebook.com/${facebook}`} target="_blank" rel="noopener noreferrer"
                        className={`w-9 h-9 rounded-xl flex items-center justify-center transition-colors ${isDark ? 'bg-slate-700 hover:bg-slate-600' : 'bg-slate-100 hover:bg-slate-200'}`} aria-label="Facebook">
                    ...
                    {tiktok && (
                      <a href={`https://tiktok.com/@${tiktok}`} target="_blank" rel="noopener noreferrer"
                        className={`w-9 h-9 rounded-xl flex items-center justify-center transition-colors ${isDark ? 'bg-slate-700 hover:bg-slate-600' : 'bg-slate-100 hover:bg-slate-200'}`} aria-label="TikTok">
              ...
              {/* Powered By */}
              <div className="space-y-4">
                <h4 className={`text-sm font-bold uppercase tracking-wider ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>Menu Digital</h4>
                <div className="space-y-3">
                  <p className={`text-sm ${descColor}`}>Menu digital modern untuk bisnis kuliner Anda</p>
                  <a href="/" className="inline-flex items-center gap-2 text-sm font-semibold transition-colors" style={{ color: primaryColor }}>
            ...
            <div className={`mt-8 pt-8 border-t ${isDark ? 'border-slate-800' : 'border-slate-100'} flex flex-col sm:flex-row justify-between items-center gap-4`}>
              <p className={`text-xs ${descColor}`}>© {new Date().getFullYear()} {store.name}. All rights reserved.</p>
              <p className={`text-xs ${descColor}`}>Powered by <span className="font-semibold" style={{ color: primaryColor }}>MyMenu</span></p>
            </div>
          </div>
        </footer>
